# Comparative report - IDoFT, NonDex, iDFlakies, FlakeSync, RankF
...
Both tools cost noticeably more than "rounds × baseline" would suggest - the gap is each tool's own bookkeeping (writing `.nondex\`/`.dtfixingtools\`
...
Standard practice in this study - *"write down the null results, they're
...
- NonDex was only run against one project (`delight-nashorn-sandbox`) - no NonDex vs. IDoFT `ID`-category comparison exists yet, since none of
...
**Cross-project note, now examined (see §11 below):** `apache/dubbo` is both
a FlakeSync subject (M7–M11) and RankF's worked example project. FlakeSync's
M9 and RankF's Wei-dataset entry turn out to share the exact same module and
commit (`dubbo-rpc/dubbo-rpc-dubbo` @ `737f7a7`) and the same test class
(`DubboProtocolTest`) - real overlap, detailed below.

---

## 11. RankF - a ranking tool, not a detector, added 2026-09-23

Full detail: [rankf_findings.md](rankf_findings.md),
[rankf_report.md](reports/rankf/rankf_report.md).

RankF is the natural next step after iDFlakies: where iDFlakies can *detect*
that `marine-api`'s 12 confirmed tests are order-dependent but (per §3 above)
labels all 12 generically, RankF is supposed to *diagnose* which other test
is responsible. Running it directly tested whether that gap actually closes.

| Connection to earlier findings | What RankF adds |
| --- | --- |
| Extends §5's "false success" pattern to a new failure shape | Maven Surefire's `runOrder=random` only shuffles test *classes*, never methods *within* one - for `marine-api`, whose real polluter and cleaner share a class, this makes it **structurally impossible**, not just unlikely, to generate a failing test-order corpus. Proven empirically (90 seeds, 0 failures) and then directly (identical intra-class order at every seed) |
| Confirms §3's iDFlakies/IDoFT finding matters downstream, not just as a labeling curiosity | iDFlakies' generic `"OD"` label (no victim/brittle distinction) is exactly the one input RankF's own algorithm needs to pick the right scoring branch. Demonstrated concretely: guessing wrong drops the true polluter from rank 1 to rank 17 on the identical corpus |
| A new, quantified version of §6's timing comparisons | Real measured baseline costs on the same machine: OBO ~8.6s/candidate (median), delta-debugging 4 invocations / 50.6s to converge on an 8-candidate prefix. Costing the paper's own admitted 86–206-order requirement for a *stable* `RankF_O` ranking against these gives 18.5–44.3 minutes - 22–52× delta-debugging's real cost, directly undercutting the paper's "cheap alternative" framing (with the honest counterpoint that a corpus amortizes across many OD tests, unlike delta-debugging) |
| The `apache/dubbo` overlap with FlakeSync, resolved | FlakeSync's M9 and RankF's Wei-dataset entry share module `dubbo-rpc/dubbo-rpc-dubbo` and SHA `737f7a7`, and both flag the same class, `DubboProtocolTest`, as flaky - but by different mechanisms (order-dependence vs. async timing) and on different specific methods within that class. Not a direct same-test contradiction, but a closer, unexamined-by-either-paper overlap than expected |

**Cross-check into §5's meta-pattern:** RankF adds a fourth instance of "a
tool's default configuration produces a materially different, usually
weaker, result than its stress-tested configuration" - the same shape as
FlakeSync's shipped delay schedule (§10) and the iDFlakies default-round
finding, now shown on a ranking technique via a real order-count sweep: at
the paper's own evaluated 20 orders, 3 independent corpus shufflings failed
to find the true polluter 3 out of 3 times.
